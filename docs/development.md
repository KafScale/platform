...
- The Helm chart defaults image tags to `appVersion`, so bump `deploy/helm/kafscale/Chart.yaml` `version` and `appVersion` for each release. Users can still override `operator.image.tag` and `console.image.tag` to pin a specific version.
